...
int main() {
    int start;
    printf("\t\t**WELCOME TO JAYPEE PARKING SYSTEM**\n\n");
...
int user_Details() {
    data = fopen("D:\\Entry_details2.txt","a");
    printf("\nEnter name: ");
    scanf("%s",user.name);
...
int searcher(char vehicle_details_matcher[100]) {
    data = fopen("D:\\Entry_details2.txt","r");
    Backlog   = fopen("D:\\backlog.txt","a");
    time_t t;
    time(&t);
    //time_t seconds;
...
{
    readdata();
    data=fopen("D:\\Entry_details2.txt","w");
    for (int w=0;w<t;w++)
    {
